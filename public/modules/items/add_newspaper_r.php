    ...
    try {
        ...
        if (empty($newspaper_name)) {
            ...
            if ($check->fetch()) {
                ...
                $stmt = $pdo->prepare("INSERT INTO items (lang, type, title, issue_from, issue_to, newspaper_date, missing_issues, box_number, cabinet_number, shelf_number, drawer_number, registration_date, modification_date, field, newspaper_type, created_at) VALUES (?, 'newspaper', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'General', 'official', NOW())");
...
$newspaper_options = [
    'ar' => ['shaab' => 'الجريدة الرسمية'],
    'fr' => ['shaab' => 'Journal Officiel'],
];
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?= $lang == 'ar' ? 'إضافة الجريدة الرسمية' : 'Ajouter Journal Officiel' ?> - نظام المكتبة</title>
...
<div class="header">
    ...
    <h1 class="header-title"><?= $lang == 'ar' ? 'إضافة الجريدة الرسمية' : 'Ajouter Journal Officiel' ?></h1>
    <p class="header-subtitle"><?= $lang == 'ar' ? 'تسجيل مجموعة أعداد من الجريدة الرسمية' : 'Enregistrer une série de numéros du journal officiel' ?></p>
</div>

<?php
/* modules/items/list_magazines.php */
session_start();
...
if ($db_connected) {
    try {
        ...
        if (!empty($search)) {
            if ($search_field === 'all') {
                $where[]             = "(title LIKE :search OR magazine_type LIKE :search OR publisher LIKE :search OR issue_number LIKE :search OR issn LIKE :search OR publication_place LIKE :search)";
        ...
        $sql = "SELECT id, title, magazine_type, issue_number, publication_date, publisher,
                       publication_place, issn, classification_number, magazine_number,
                       cabinet_number, shelf_number, drawer_number, registration_date,
                       modification_date, field, lang, created_at, periodical_file
...
$total_pages = max(1, ceil($total_magazines / $limit));
...
<head>
    ...
    <title><?= $lang == 'ar' ? 'الدوريات والمجلات' : 'Revues et Périodiques' ?> - نظام المكتبة</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .search-input:focus { outline:none; border-color:var(--primary); box-shadow:0 0 0 3px rgba(8,145,178,.12); }
        ...
        .filter-label { font-weight:600; color:var(--gray-700); font-size:.875rem; display:flex; align-items:center; gap:.4rem; }
        ...
        .btn-success:hover { background:#16a34a; transform:translateY(-2px); box-shadow:var(--shadow-md); }
        ...
        .mag-title { font-weight:600; color:var(--gray-900); }
        .mag-type  { color:var(--gray-600); font-size:.8rem; margin-top:.2rem; }
        ...
        @media (max-width: 768px) {
            ...
            .search-form, .filter-row { flex-direction:column; align-items:stretch; }
...
<div class="page-header">
    <div class="header-content">
        ...
        <h1 class="header-title"><?= $lang == 'ar' ? 'الدوريات والمجلات' : 'Revues et Périodiques' ?></h1>
        <p class="header-subtitle"><?= $lang == 'ar' ? 'إدارة وعرض الدوريات والمجلات المتخصصة' : 'Gestion et affichage des revues et magazines spécialisés' ?></p>
    </div>
</div>
...
<div class="container">
    ...
    <div class="search-section fade-in" style="animation-delay:.1s">
        <form method="GET" class="search-form">
            ...
            <select name="search_field" class="search-select">
                ...
                <option value="title"            <?= $search_field === 'title'            ? 'selected' : '' ?>><?= $lang == 'ar' ? 'العنوان'       : 'Titre' ?></option>
                <option value="magazine_type"    <?= $search_field === 'magazine_type'    ? 'selected' : '' ?>><?= $lang == 'ar' ? 'النوع'         : 'Type' ?></option>
                <option value="issue_number"     <?= $search_field === 'issue_number'     ? 'selected' : '' ?>><?= $lang == 'ar' ? 'العدد'         : 'Numéro' ?></option>
                <option value="issn"             <?= $search_field === 'issn'             ? 'selected' : '' ?>>ISSN</option>
                <option value="publisher"        <?= $search_field === 'publisher'        ? 'selected' : '' ?>><?= $lang == 'ar' ? 'دار النشر'     : 'Éditeur' ?></option>
                <option value="publication_place"<?= $search_field === 'publication_place'? 'selected' : '' ?>><?= $lang == 'ar' ? 'مكان النشر'    : 'Lieu de pub.' ?></option>
            </select>
    ...
    <?php if (!empty($search)): ?>
        <div class="search-info fade-in" style="animation-delay:.15s">
            ...
            <?= $lang == 'ar' ? 'نتائج البحث عن:' : 'Résultats pour:' ?> <strong>"<?= htmlspecialchars($search) ?>"</strong>
    ...
    <div class="stats-bar fade-in" style="animation-delay:.2s">
        <div class="stat-item">
            ...
            <div class="stat-info">
                <h4><?= $lang == 'ar' ? 'إجمالي الدوريات' : 'Total des revues' ?></h4>
                <p><?= $total_magazines ?></p>
            </div>
        </div>
        <div class="stat-item">
    ...
    <div class="table-card fade-in" style="animation-delay:.3s">
        <div class="table-header">
            <h3>
                ...
                <?= $lang == 'ar' ? 'قائمة الدوريات' : 'Liste des revues' ?>
                <?php if (!empty($search)): ?>
        ...
        <?php if (empty($magazines)): ?>
            <div class="empty-state">
                ...
                <h3 style="color:var(--gray-700);margin-bottom:.5rem;">
                    <?= $lang == 'ar' ? 'لا توجد دوريات' : 'Aucune revue trouvée' ?>
                </h3>
                <p><?= (!empty($search) || !empty($field_filter))
                    ? ($lang == 'ar' ? 'لم يتم العثور على نتائج لبحثك' : 'Aucun résultat pour votre recherche')
                    : ($lang == 'ar' ? 'لم يتم إضافة أي دوريات بعد'  : "Aucune revue n'a été ajoutée pour le moment") ?></p>
            </div>
        <?php else: ?>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th><?= $lang == 'ar' ? 'العدد' : 'N°' ?></th>
                            <th>ISSN</th>
                            <th><?= $lang == 'ar' ? 'تاريخ النشر' : 'Date pub.' ?></th>
                            <th><?= $lang == 'ar' ? 'دار النشر' : 'Éditeur' ?></th>
                            <th><?= $lang == 'ar' ? 'مكان النشر' : 'Lieu de pub.' ?></th>
                            <th><?= $lang == 'ar' ? 'تفاصيل' : 'Détails' ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($magazines as $idx => $m):
                            $row_id = 'mag-det-' . (int)$m['id'] . '-' . $idx;
                        ?>
                        <tr class="mag-row">
                            <td style="font-weight:600;color:var(--primary);"><?= $offset + $idx + 1 ?></td>
                            <td><div class="mag-title"><?= htmlspecialchars($m['title']) ?></div></td>
                            <td><?= !empty($m['magazine_type']) ? htmlspecialchars($m['magazine_type']) : '-' ?></td>
                            <td><?= htmlspecialchars($m['issue_number'] ?? '-') ?></td>
                            <td style="font-family:monospace;font-size:.8rem;"><?= htmlspecialchars($m['issn'] ?? '-') ?></td>
                            <td><?= $m['publication_date'] ? date('Y-m-d', strtotime($m['publication_date'])) : '-' ?></td>
                            <td><?= htmlspecialchars($m['publisher'] ?? '-') ?></td>
                            <td><?= htmlspecialchars($m['publication_place'] ?? '-') ?></td>
                            <td>
                            ...
                            <td colspan="9">
                                <div class="details-card">
                                    <div class="details-title">
                                        <i class="fas fa-newspaper" style="color:var(--primary);margin-<?= $lang=='ar'?'left':'right' ?>:.4rem;"></i>
                                        <?= $lang == 'ar' ? 'المعلومات التفصيلية للدورية' : 'Informations détaillées de la revue' ?>
                                    </div>
                                    <div class="details-grid">
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'رقم التصنيف' : 'Classification' ?></strong>
                                            <span><?= htmlspecialchars($m['classification_number'] ?? '-') ?></span>
                                        </div>
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'رقم المجلة' : 'N° revue' ?></strong>
                                            <span><?= htmlspecialchars($m['magazine_number'] ?? '-') ?></span>
                                        </div>
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'رقم الخزانة' : 'N° armoire' ?></strong>
                                            <span><?= htmlspecialchars($m['cabinet_number'] ?? '-') ?></span>
                                        </div>
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'رقم الرف' : 'N° étagère' ?></strong>
                                            <span><?= htmlspecialchars($m['shelf_number'] ?? '-') ?></span>
                                        </div>
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'رقم الدرج' : 'N° tiroir' ?></strong>
                                            <span><?= htmlspecialchars($m['drawer_number'] ?? '-') ?></span>
                                        </div>
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'تاريخ التسجيل' : 'Date d\'enregistrement' ?></strong>
                                            <span><?= htmlspecialchars($m['registration_date'] ?? '-') ?></span>
                                        </div>
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'تاريخ التعديل' : 'Date de modification' ?></strong>
                                            <span><?= htmlspecialchars($m['modification_date'] ?? '-') ?></span>
                                        </div>
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'تاريخ الإضافة' : 'Date d\'ajout' ?></strong>
                                            <span><?= htmlspecialchars($m['created_at'] ?? '-') ?></span>
                                        </div>
                                        <?php if (!empty($m['periodical_file'])): ?>
                                        <div class="detail-item">
                                            <strong><?= $lang == 'ar' ? 'ملف الدورية' : 'Fichier' ?></strong>
                                            <a href="../../uploads/magazines/<?= htmlspecialchars($m['periodical_file']) ?>"
                                               target="_blank" class="file-link">
                                                <i class="fas fa-file-pdf"></i>
                                                <?= $lang == 'ar' ? 'فتح الملف' : 'Ouvrir le fichier' ?>
                                            </a>
                                        </div>
                                        <?php endif; ?>
                                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    // Auto-search on typing (debounce 500ms)

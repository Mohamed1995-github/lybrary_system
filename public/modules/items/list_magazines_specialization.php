...
// Récupérer les magazines de cette spécialisation
try {
    $field_name = ucfirst($specialization);
    
    // Compter le total pour cette spécialisation
    $count_stmt = $pdo->prepare("SELECT COUNT(*) as total FROM items WHERE type = 'magazine' AND field = ? AND lang = ?");
    $count_stmt->execute([$field_name, $lang]);
    ...
    $stmt = $pdo->prepare("
        ...
        FROM items 
        WHERE type = 'magazine' AND field = ? AND lang = ? 
        ORDER BY title ASC 
...
$translations = [
    'ar' => [
        'title' => 'المجلات المتخصصة',
        'subtitle' => 'قائمة المجلات المتخصصة',
        ...
        'no_magazines_msg' => 'لم يتم إضافة أي مجلات في هذا التخصص بعد',
        'add_first_magazine' => 'إضافة أول مجلة',
        'back_to_dashboard' => 'العودة للوحة التحكم'
    ],
    'fr' => [
        'title' => 'Revues Spécialisées',
        'subtitle' => 'Liste des revues spécialisées',
        ...
        'no_magazines_msg' => 'Aucune revue n\'a été ajoutée dans cette spécialisation pour le moment',
...
<head>
    ...
    <title><?= $current_specialization[$lang] ?> - <?= $t['title'] ?></title>
...
<body>
    <div class="container">
        ...
        <div class="stats-bar">
            ...
            <div class="stats-info">
                <a href="add_magazine_simple.php?specialization=<?=$specialization?>&category=<?=$category?>&lang=<?=$lang?>" style="display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.5rem 1rem; background: #667eea; color: white; text-decoration: none; border-radius: 6px; font-size: 0.9rem;">
        ...
        <?php if (empty($magazines)): ?>
            <div class="empty-state">
                ...
                <a href="add_magazine_simple.php?specialization=<?=$specialization?>&category=<?=$category?>&lang=<?=$lang?>" style="display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1.5rem; background: #667eea; color: white; text-decoration: none; border-radius: 8px; margin-top: 1rem;">
                    <i class="fas fa-plus"></i>
